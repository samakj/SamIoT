...
{
    if (!_data.count(id)) _data.insert(std::make_pair(id, ExecutionTimeData()));
    _data[id].start();
};
...
{
    if (_lastLoopReportMillis == -1) _lastLoopReportMillis = millis();
    if (TimeUtils.millisSince(_lastLoopReportMillis) / 1000.0f > reportPeriod)
    {
        ...
        Log.debug("  | ID       | min (ms) | max (ms) | ave (ms) | % of total |");
