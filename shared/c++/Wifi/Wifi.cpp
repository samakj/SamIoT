...
namespace SamIoT::Wifi
{
    ...
    float STRENGTH_NULL_VALUE = -1.0f;
    uint8_t IP_LOCATION_NULL_VALUE = 0;
    std::string ssid = SamIoT::Wifi::SSID_NULL_VALUE;
    float strength = SamIoT::Wifi::STRENGTH_NULL_VALUE;
    uint16_t strengthUpdatePeriod = 10000;
    std::vector<SamIoT::Wifi::ConnectCallback> connectCallbacks = {};
    std::vector<SamIoT::Wifi::SsidCallback> ssidCallbacks = {};
    std::vector<SamIoT::Wifi::StrengthCallback> strengthCallbacks = {};
    unsigned long lastStrengthUpdate = 0;
    ...
    SamIoT::Wifi::WifiCredentials *getStrongestNetwork(std::vector<WifiCredentials *> credentials)
    {
        if (!credentials.size())
        {
