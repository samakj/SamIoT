...
float AsyncWifiClass::getConnectedNetworkStrength()
{
    if (TimeUtils.millisSince(_lastStrengthUpdate) > (unsigned long)(strengthUpdatePeriod * 1000))
    {
        if (!isConnected())
        {
            Sam::Log.error("Not connected to network, cannot get strength.");
...
{
    Sam::Log.debugf("Finding strength of %s network...\n", ssid.c_str());
    Sam::Log.debug("Scanning local networks...");

    int networkCount = WiFi.scanNetworks();

    Sam::Log.debugf("%d networks found in range.\n", networkCount);
    ...
    for (int i = 0; i < networkCount; i++)
    {
        if (ssid == WiFi.SSID(i).c_str())
        {
            ...
            Sam::Log.debugf("'%s' network found with strength %.1f%%.\n\n", ssid.c_str(), strength * 100);
            return strength;
        }
    }

    Sam::Log.warnf("'%s' network not found.\n\n", ssid.c_str());
...
WifiCredentials *AsyncWifiClass::getStrongestNetwork(std::vector<WifiCredentials *> credentials)
{
    if (!credentials.size())
    {
        Sam::Log.error("Empty list passed to getStrongestNetwork");
        return nullptr;
    }
    ...
    int maxWait)
{
    if (!isConnected())
    {
        ...
        if (strongest)
            _connect(strongest, hostname, ipLocation, maxWait);
    }
}
    ...
    int maxWait)
{
    ...
    while (!isConnected())
    {
        ...
        {
            Sam::Log.warn("Max wait exceeded.", "\n");
            return;
        }
        Sam::Log.infof(
